...
def getActorPhoto(html: str) -> dict:  # //*[@id="star_qdt"]/li/a/img
    ...
    if results:
        ...
        actor = ar.findall(item)
        actor_photo = {}
        for i in actor:
            actor_photo[i[1]] = getOnePhoto('https://javdb.com' + i[0])
        return actor_photo
    else:
        return {}
...
def main(number):
    ...
    if re.search(r'[a-zA-Z]+\.\d{2}\.\d{2}\.\d{2}', number):
        cover_small = getCoverSmall(query_result)
    else:
        cover_small = getCoverSmall(query_result, index=ids.index(number))
